...
func InitRoutes(router *gin.Engine, handler *handlers.Handler) {
	...
	classes.Use(middleware.RequireAuth(handler.Database))
	{
		...
		classes.DELETE("delete-user/:class-id/:user-id", handler.DeleteUserFromClassHandler)
		...
		classes.GET("/assignment/:id", handler.GetAssignmentHandler)
		...
		classes.GET("mark/:assignment-id", handler.GetMarkHandler)
